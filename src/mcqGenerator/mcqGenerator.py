...
llm = ChatOpenAI(base_url=endpoint, api_key=api_key,
                 model_name=model_name, temperature=0.8)
...
quiz_generation_prompt = PromptTemplate(input_variables=[
                                        "text", "number", "subject", "tone", "response_json"], template=TEMPLATE)
...
You need to evaluate the complexity of the question and give only review of the quiz, using maximum of 50 tokens for complexity analysis. Do not include anything like corrected/updated mcqs, quiz analysis etc in the review just include the review of quiz itself. Just give complexity analysis & review of the quiz.
...
quiz_evaluation_prompt = PromptTemplate(
    input_variables=["subject", "quiz"], template=TEMPLATE_2)
